...
namespace PNET_semestralka.Controllers
{
    public class ProductsManagingController : Controller
    {
		...
		{
			var product = _productService.GetProductById(data.Id);
			if (product == null)
		...
		public IActionResult AddProduct([FromForm] ProductDto data)
		{
			...
			// Vložení do databáze
			_productService.AddNewProduct(data.Nazev, data.Popis, data.Cena, data.Mnozstvi, seller, fileName);
		...
		public IActionResult EditProduct([FromForm] ProductDto data)
		{
			...
			_productService.UpdateProduct(data.Id, data.Nazev, data.Popis, data.Cena, data.Mnozstvi, data.UseExistingImage, fileName);
